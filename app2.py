...
code = st.text_area("请输入代码或任务：", height=300, key="code_input_area")
mode = st.radio("选择分析模式：", ["⚡ 快速分析", "🌊 深度分析"])
enable_thinking = mode.startswith("🌊")

# ✅ 模型选择 + API Key（仅在线模型显示）
model_mode = st.selectbox("选择模型来源：", ["💻 本地模型（Qwen3）", "☁️ 在线模型（DeepSeek Chat）"])
use_local_model = model_mode.startswith("💻")


api_key = ""
if not use_local_model:
    api_key = st.text_input("🔑 请输入 DeepSeek Chat API Key：", type="password")
    if not api_key:
        st.warning("⚠️ 请选择在线模型后，请输入有效的 API Key")
...
def call_deepseek_api(prompt, api_key):
    ...
    try:
        ...
        yield f"❌ DeepSeek API 请求失败: {str(e)}"
...
def stream_generate_response(prompt, enable_thinking, max_new_tokens=NEW_MAX_TOKENS):
    if use_local_model:
        ...
        yield from call_deepseek_api(prompt, api_key)
